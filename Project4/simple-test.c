...
int main(int argc, char *argv[]) {
  ...
  fn = "/first-file";
  if (File_Create(fn) < 0) {
    printf("ERROR: can't create file '%d'\n", osErrno);
  } else{
  ...
  fn = "/first-file";
  if (File_Unlink(fn) < 0) {
    printf("ERROR: can't unlink file '%s'\n", fn);
  } else{
  ...
  fn = "/first-dir";
  if (Dir_Unlink(fn) < 0) {
    printf("ERROR: can't unlink dir '%s'\n", fn);
  } else{
    printf("dir '%s' unlinked successfully\n", fn);
  }
  ...
  if (File_Write(fd, buf, 1024) != 1024) {
    ...
    printf("successfully wrote 1024 bytes to fd=%d\n", fd);
  }
